...
void Graph::FordFulkerson() {
    ...
    while (BFS(path)) {
        int minResidualCapacity = std::numeric_limits<int>::max();
...
unsigned int Graph::getVertexDegree(unsigned int vertex) {
  unsigned int degree = 0;

  RepresentationType prevType = matrix->getRepresentationType();

  convertMatrix(AdjacencyMatrix);

  for (unsigned int c=0; c<matrix->getRows(); ++c) {
    if (matrix->getElement(vertex, c) == 1) {
      degree++;
    }
  }

  for (unsigned int r=0; r<matrix->getRows(); ++r) {
    if (matrix->getElement(r, vertex) == 1)
      degree++;
  }

  convertMatrix(prevType);

  return degree;
}

void Graph::multiply(std::vector<float> p, std::vector<std::vector <float>> P) {
...
std::vector<float> Graph::PageRankPt() {
  ...
  // Initialize p vector
  std::vector<float> p(n, 1.0/n);

  // Initialize output link vector
  ...
  // Make the matrix stochastic
	for (unsigned int i=0; i<n; i++){
		...
		if (out_link[i] == 0) {
			...
			for (unsigned int j=0; j<n; j++) matrix->setElement(i, j, 1.0/n);
    }

		else {
			for (unsigned int j=0; j<n; j++) {
				if (matrix->getElement(i,j) != 0) matrix->setElement(i, j, matrix->getElement(i,j)/out_link[i]);
			}
		}
	}
  ...
  std::vector<float> p_new(n, 0.0);

  while (looping) {
    // Update p_new (without using the damping factor)
		for (unsigned int i=0; i<n; i++)
			for (unsigned int j=0; j<n; j++)
				p_new[i] = p_new[i] + (transposed.matrix->getElement(i,j) * p[j]);

    // Update p_new (using the damping factor)
		for(unsigned int i=0; i<n; i++) p_new[i] = d * p_new[i] + (1.0 - d) / n;

    float error = 0.0;
    // TERMINATION: check if we have to stop
    for (unsigned int i=0; i<n; i++) error =  error + fabs(p_new[i] - p[i]);

    // if two consecutive instances of pagerank vector are almost identical, stop
    if (error < 0.000001) looping = 0;

	  // Update p vector
	  for (unsigned int i=0; i<n;i++) p[i] = p_new[i];
